...
export async function parseDXF(filePath: string): Promise<any> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (result.success) {
        ...
        console.log(`Successfully parsed CAD blueprint using Python AI-Service (walls: ${parsedData.walls?.length || 0}, rooms: ${parsedData.rooms?.length || 0}, apertures: ${parsedData.apertures?.length || 0})`);
        return {
          rooms: parsedData.rooms || [],
          walls: parsedData.walls || [],
          apertures: parsedData.apertures || [],
          furniture: parsedData.furniture || []
        };
      }
    }
  } catch (err) {
    console.warn("Python AI-Service parser is offline or failed. Falling back to local JS parser.");
  }
  ...
  if (!pythonSuccess) {
    ...
    // Arc variables
    let centerX = 0, centerZ = 0, radius = 0, startAngle = 0, endAngle = 0;
    ...
    const flushEntity = () => {
      ...
      if (currentEntity === 'LINE') {
        ...
        const steps = Math.max(6, Math.ceil(diff / (15 * Math.PI / 180)));
        const arcPoints: {x: number, z: number}[] = [];
        for (let step = 0; step <= steps; step++) {
          const theta = startRad + (diff * step) / steps;
          arcPoints.push({
        ...
        if (layer.includes('wall') && arcPoints.length > 1) {
          for (let idx = 0; idx < arcPoints.length - 1; idx++) {
            rawWalls.push({
              startX: arcPoints[idx].x,
              startZ: arcPoints[idx].z,
              endX: arcPoints[idx+1].x,
              endZ: arcPoints[idx+1].z
            });
          }
        } else if (layer.includes('door')) {
          rawDoors.push({ type: 'door', x: centerX, z: centerZ, width: radius });
        } else if (layer.includes('window')) {
          rawWindows.push({ type: 'window', x: centerX, z: centerZ, width: radius });
        }
      ...
      currentPolyPoints = [];
      isClosedPoly = false;
      centerX = 0; centerZ = 0; radius = 0; startAngle = 0; endAngle = 0;
    };
    ...
    while (i < lines.length) {
      ...
      if (code === 0) {
        if (val === 'VERTEX' && currentEntity === 'POLYLINE') {
          // Vertex entity of a heavy POLYLINE - do not flush
          continue;
        }
        ...
        if (val === 'SEQEND') {
          currentEntity = null;
        } else if (['LINE', 'LWPOLYLINE', 'POLYLINE', 'ARC', 'TEXT', 'MTEXT'].includes(val)) {
          currentEntity = val;
        } else {
          currentEntity = null;
        }
        ...
        if (code === 8) {
          ...
          if (currentEntity === 'LWPOLYLINE' || currentEntity === 'POLYLINE') {
            ...
            centerX = parseFloat(val);
          }
        } else if (code === 20) {
          startZ = parseFloat(val);
          if ((currentEntity === 'LWPOLYLINE' || currentEntity === 'POLYLINE') && currentPolyPoints.length > 0) {
            currentPolyPoints[currentPolyPoints.length - 1].z = parseFloat(val);
          } else if (currentEntity === 'TEXT' || currentEntity === 'MTEXT') {
            textZ = parseFloat(val);
          } else if (currentEntity === 'ARC') {
            centerZ = parseFloat(val);
          }
        } else if (code === 11) {
          endX = parseFloat(val);
        } else if (code === 21) {
          ...
          textVal = val;
        } else if (code === 40 && currentEntity === 'ARC') {
          radius = parseFloat(val);
        } else if (code === 50 && currentEntity === 'ARC') {
          startAngle = parseFloat(val);
        } else if (code === 51 && currentEntity === 'ARC') {
          endAngle = parseFloat(val);
        } else if (code === 70 && (currentEntity === 'LWPOLYLINE' || currentEntity === 'POLYLINE')) {
